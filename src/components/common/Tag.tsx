import { BsCheck, BsCursor, BsHash, BsJournalText, BsX } from 'react-icons/bs'
...
const getStyles = (type?: string) => {
	...
	switch (type) {
		case 'select': {
			...
			obj.icon = <BsCursor className="my-auto mr-1 h-4 w-4" />
			break
		}
		case 'checkbox': {
		...
		case 'text': {
			...
			obj.icon = <BsJournalText className="my-auto mr-1 h-4 w-4" />
			break
		}
		case 'number': {
...
export const Tag = ({ label, className, type }: TagProps) => {
	const css = ['flex rounded-lg p-1 pl-2 pr-3 text-sm dark:text-dark-300', className]
